...
export default function Sidebar({ activeMenu, setActiveMenu, isOpen, setIsOpen }) {
  return (
    <aside 
      // Logika Responsif: Melayang (fixed) di HP, Relatif di Desktop. Geser dengan transform.
    ...
    >
      <div className="h-20 flex items-center justify-between px-8 border-b border-(--border-subtle) shrink-0 transition-colors">
        <div className="flex items-center gap-2 text-2xl font-bold text-green-800 dark:text-green-500 tracking-tight">
          <Heart className="fill-green-600 dark:fill-green-500 text-green-600 dark:text-green-500" size={28} /> FitSmart
        </div>
        {/* Tombol Tutup (X) - Hanya muncul di Mobile */}
        <button 
          className="md:hidden text-(--text-muted)r:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded transition-colors p-1"
            ...
            {menuItems.map((item) => (
              <li key={item.id}>
                <button 
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${activeMenu === item.id ? 'bg-green-700 text-white shadow-md shadow-green-700/20' : 'text-(--text-muted) hover:bg-(--bg-hover) hover:text-(--accent-primary)'}`}
                >
